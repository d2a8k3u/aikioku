...
class TestGetStats:
    def test_returns_all_counts_and_version(self, client):
        with (
            patch(
                "src.api.stats._get_note_count", new_callable=AsyncMock, return_value=5
            ) as mock_notes,
            patch(
                "src.api.stats._get_entity_count", new_callable=AsyncMock, return_value=3
            ) as mock_entities,
            patch(
                "src.api.stats._get_relation_count", new_callable=AsyncMock, return_value=2
            ) as mock_relations,
            patch(
                "src.api.stats._get_memory_count", new_callable=AsyncMock, return_value=10
            ) as mock_memories,
