...
def _make_results(source: str, note_ids: list[str], scores: list[float] | None = None) -> list[SearchResult]:
    """Helper to build a list of SearchResult for a given source."""
    if scores is None:
        scores = [1.0 - i * 0.1 for i in range(len(note_ids))]
    return [
        SearchResult(note_id=nid, score=sc, source=source)
        for nid, sc in zip(note_ids, scores)
    ]
...
@pytest.mark.asyncio
async def test_search_runs_all_retrievers_and_returns_top(fusion, mock_dense, mock_sparse, mock_graph):
...
async def test_fuse_prefers_non_empty_snippet(fusion):
    ...
    results = {
        ...
        "sparse": [
            SearchResult(note_id="A", score=0.8, source="sparse", snippet="real content")
        ],
    }
    fused = fusion._fuse(results)
